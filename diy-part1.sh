...
mkdir -p files/etc/
touch files/etc/hackxiu_version
mkdir wget
touch wget/DISTRIB_REVISION1
touch wget/DISTRIB_REVISION3
touch files/usr/share/Check_Update.sh
touch files/usr/share/hackxiu-auto.sh
touch files/usr/share/hackxiu-pw.sh
...
cat >rename.sh <<-\EOF
	...
	sleep 2
	rename_version=`cat files/etc/hackxiu_version`
	...
	if [ "$str1" = "5.4" ];then
	   mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined.img.gz       bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver54}_dev_hackxiu.img.gz
	  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined-efi.img.gz   bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver54}_uefi-gpt_dev_hackxiu.img.gz
	elif [ "$str1" = "5.10" ];then
	   mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined.img.gz       bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver510}_dev_hackxiu.img.gz
	  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined-efi.img.gz   bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver510}_uefi-gpt_dev_hackxiu.img.gz
	elif [ "$str1" = "5.15" ];then
	   mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined.img.gz       bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver515}_dev_hackxiu.img.gz
	  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined-efi.img.gz   bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver515}_uefi-gpt_dev_hackxiu.img.gz
	elif [ "$str1" = "6.1" ];then
	  if [ ! $ver66 ]; then
	  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined.img.gz       bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver61}0_dev_hackxiu.img.gz
	  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined-efi.img.gz   bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver61}0_uefi-gpt_dev_hackxiu.img.gz
	 else
	  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined.img.gz       bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver61}_dev_hackxiu.img.gz
	  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined-efi.img.gz   bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver61}_uefi-gpt_dev_hackxiu.img.gz
	   fi 
	elif [ "$str1" = "6.6" ];then
	  if [ ! $ver66 ]; then
	  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined.img.gz       bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver66}0_dev_hackxiu.img.gz
	  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined-efi.img.gz   bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver66}0_uefi-gpt_dev_hackxiu.img.gz
	 else
	  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined.img.gz       bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver66}_dev_hackxiu.img.gz
	  mv  bin/targets/x86/64/openwrt-x86-64-generic-squashfs-combined-efi.img.gz   bin/targets/x86/64/openwrt_x86-64-${rename_version}_${str1}.${ver66}_uefi-gpt_dev_hackxiu.img.gz
	   fi
	fi
	ls bin/targets/x86/64 | grep "gpt_dev_hackxiu.img" | cut -d - -f 3 | cut -d _ -f 1-2 > wget/op_version1
	#md5
	ls -l  "bin/targets/x86/64" | awk -F " " '{print $9}' > wget/open_dev_md5
	dev_version=`grep "_uefi-gpt_dev_hackxiu.img.gz" wget/open_dev_md5 | cut -d - -f 3 | cut -d _ -f 1-2`
	openwrt_dev=openwrt_x86-64-${dev_version}_dev_hackxiu.img.gz
	openwrt_dev_uefi=openwrt_x86-64-${dev_version}_uefi-gpt_dev_hackxiu.img.gz
...
cat >hackxiu.sh <<-\EOOF
	...
	if [ $? != 0 ]; then
		...
		sed -i '$ a alias hackxiu="sh /usr/share/Check_Update.sh"' /etc/profile
		chmod 755 /etc/init.d/romupdate
		exit 0
		EOF
	fi
	grep "hackxiu-auto.sh"  package/lean/default-settings/files/zzz-default-settings
	if [ $? != 0 ]; then
		sed -i 's/exit 0/ /'  package/lean/default-settings/files/zzz-default-settings
		cat>> package/lean/default-settings/files/zzz-default-settings<<-EOF
		sed -i '$ a alias hackxiu-auto="sh /usr/share/hackxiu-auto.sh"' /etc/profile
		chmod 755 /etc/init.d/romupdate
		exit 0
		EOF
	fi

	grep "hackxiu-pw.sh"  package/lean/default-settings/files/zzz-default-settings
	if [ $? != 0 ]; then
		sed -i 's/exit 0/ /'  package/lean/default-settings/files/zzz-default-settings
		cat>> package/lean/default-settings/files/zzz-default-settings<<-EOF
		sed -i '$ a alias hackxiu-pw="sh /usr/share/hackxiu-pw.sh"' /etc/profile
		chmod 755 /etc/init.d/romupdate
		exit 0
		EOF
	fi
...
cat >files/usr/share/Check_Update.sh <<-\EOF
	#!/bin/bash
	# https://github.com/hackxiu/Actions-OpenWrt-x86
	# Actions-OpenWrt-x86 By hackxiu 20210505
	#path=$(dirname $(readlink -f $0))
	# cd ${path}
	#检测准备
	if [ ! -f  "/etc/hackxiu_version" ]; then
		echo
		echo -e "\033[31m 该脚本在非hackxiu固件上运行，为避免不必要的麻烦，准备退出… \033[0m"
		echo
		exit 0
	fi
	rm -f /tmp/cloud_version
	# 获取固件云端版本号、内核版本号信息
	current_version=`cat /etc/hackxiu_version`
	wget -qO- -t1 -T2 "https://api.github.com/repos/hackxiu/Actions-OpenWrt-x86/releases/latest" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g;s/v//g'  > /tmp/cloud_ts_version
	if [ -s  "/tmp/cloud_ts_version" ]; then
		...
		DEV_URL=https://github.com/hackxiu/Actions-OpenWrt-x86/releases/download/${new_version}/openwrt_x86-64-${new_version}_dev_hackxiu.img.gz
		DEV_UEFI_URL=https://github.com/hackxiu/Actions-OpenWrt-x86/releases/download/${new_version}/openwrt_x86-64-${new_version}_uefi-gpt_dev_hackxiu.img.gz
		openwrt_dev=https://github.com/hackxiu/Actions-OpenWrt-x86/releases/download/${new_version}/openwrt_dev.md5
		openwrt_dev_uefi=https://github.com/hackxiu/Actions-OpenWrt-x86/releases/download/${new_version}/openwrt_dev_uefi.md5
	else
		echo "请检测网络或重试！"
		exit 1
	fi
	...
	echo $Firmware_Type > /etc/hackxiu_firmware_type
	echo
	if [[ "$cloud_kernel" =~ "4.19" ]]; then
		echo
		echo -e "\033[31m 该脚本在hackxiu固件Sta版本上运行，目前只建议在Dev版本上运行，准备退出… \033[0m"
		echo
		exit 0
	fi
	#md5值验证，固件类型判断
	if [ ! -d /sys/firmware/efi ];then
		if [ "$current_version" != "$cloud_version" ];then
			wget -P /tmp "$DEV_URL" -O /tmp/openwrt_x86-64-${new_version}_dev_hackxiu.img.gz
		...
		if [ "$current_version" != "$cloud_version" ];then
			wget -P /tmp "$DEV_UEFI_URL" -O /tmp/openwrt_x86-64-${new_version}_uefi-gpt_dev_hackxiu.img.gz
	...
	case $num1 in
	  ...
	  echo -e "\033[32m >>>正在准备保留配置升级，请稍后，等待系统重启…-> \033[0m"
		...
		sleep 3
		if [ ! -d /sys/firmware/efi ];then
			sysupgrade /tmp/openwrt_x86-64-${new_version}_dev_hackxiu.img.gz  
		else
			sysupgrade /tmp/openwrt_x86-64-${new_version}_uefi-gpt_dev_hackxiu.img.gz
		fi
	    ;;
	    n|N)
	    echo
	    ...
	    sleep 3
		if [ ! -d /sys/firmware/efi ];then
			sysupgrade -n  /tmp/openwrt_x86-64-${new_version}_dev_hackxiu.img.gz
		else
			sysupgrade -n  /tmp/openwrt_x86-64-${new_version}_uefi-gpt_dev_hackxiu.img.gz
		fi
	    ;;
	    *)
		  echo
...
cat >files/usr/share/hackxiu-auto.sh <<-\EOF
	#!/bin/bash
	# https://github.com/hackxiu/Actions-OpenWrt-x86
	# Actions-OpenWrt-x86 By hackxiu 20210505
	#path=$(dirname $(readlink -f $0))
...
cat >files/usr/share/hackxiu-pw.sh <<-\EOF
	#!/bin/sh
	# Define variables
	TEMP_DIR="/tmp/test"
